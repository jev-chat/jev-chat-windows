...
> 非官方社区版本：本项目基于官方上游的
> [jev-chat-windows](https://github.com/jev-chat/jev-chat-windows) 二次开发，
> 由社区成员自行维护，与原作者没有官方隶属、授权或背书关系。
>
> 发布和再分发时请保留本仓库的 [LICENSE](LICENSE)、[NOTICE](NOTICE) 和
> [社区声明](NOTICE-COMMUNITY.md)。原项目名称和链接仅用于说明代码来源，不代表原作者参与本版本。

## 来源与本版本改动

原始项目：<https://github.com/jev-chat/jev-chat-windows>

本版本在上游 Windows 版基础上主要增加或调整了：

- 微信 / QQ 采集与填入自由切换；
- 判断方式跟随下方起草模型，支持 DeepSeek、智谱 GLM、Kimi、OpenAI、OpenRouter 等来源；
- 关系规则和场景规则内置为 Markdown，同时允许用户自定义；
- 身体不适、情绪状态、近期历史上下文和紧张度判断；
- 生成失败重试、按选中聊天记录重新生成；
- 本地文字历史记录、保留期限和数据库大小控制；
- 降低普通消息的重复判断、候选补齐和 Token 消耗。
...
要求：Windows 10 1903+ / 11，微信 Windows 4.x 或 QQ Windows，默认可只使用一把 DeepSeek API key。

首次启动会弹设置页配置模型和关系；选择 DeepSeek 判断时只需要使用起草区的 DeepSeek key，暂时不填 key 也可以先保存设置。key 写进 Windows 用户环境变量（注册表 `HKCU\Environment`）——
...
设置页可以自由选择「聊天窗口：微信 / QQ」。还可以选择「判断方式」：

1. **跟随下方起草模型（推荐）** —— 判断、起草使用同一个来源、模型和 `LLM_API_KEY`。下方选 DeepSeek、智谱 GLM、Kimi、OpenAI、OpenRouter 或自定义兼容接口都可以，不需要额外注册 OpenRouter/TypeSafe。
2. **原版 Jev** —— 保留项目原来的 OpenRouter / TypeSafe 判断链路，需要额外配置 `JEV_API_KEY`。

设置页的「模型」卡片分两节；跟随模式共用起草区这一把 key，原版 Jev 模式才需要各填一把：
...
3. 选「你们的关系」（女友/恋人、追求对象、暧昧对象、朋友、同事、家人或自定义），保存。可以用了。

跟随模式下只需要起草区当前来源的 key；原版 Jev 模式才需要两把 key。保存设置时两把 key 都可以暂时留空，
这样分享压缩包或刚重置设置后也能先保存界面配置；真正分析/生成前，程序才会检查当前模式所需的 key。DeepSeek 判断会复用
起草区的 key，上面的原版 Jev/OpenRouter/TypeSafe key 可以完全不填。换聊天窗口后保存设置，采集会自动切换到对应窗口。

注意：模型厂商的网页套餐/聊天会员不能直接当 API 使用；要让软件调用，仍需该来源提供的 API key。GLM 可以选「智谱 GLM」并填智谱 API key，
OpenRouter 可以选「OpenRouter」并填 OpenRouter API key。跟随模式只是统一调用来源，不会把网页套餐转换成 API。
...
判断那一步比起草轻得多；如果使用原版 Jev，才需要在上面的判断卡片里选 OpenRouter 或 TypeSafe 直连。
两把 key 都只进注册表，不落文件。

**日常怎么用**

- 微信或 QQ 开着、别最小化（用别的窗口盖住没事），把要聊的会话点开
- 对方来一条消息 → 悬浮窗几秒后给判断摘要 + 三条候选（带 Jev 给的胜出概率）
- 点「填入聊天窗口」→ 文字进当前输入框 → **你自己看一眼、改一改、按发送**。程序永远不碰发送
- 你在微信/QQ里切到哪个会话，悬浮窗就跟到哪个；群聊会带上发言人名，想指定回复给谁去设置里开「群聊指定回复对象」
- 生成失败时可以点「重试」；如果你发了表情包、图片或误被识别成“我已回复”，展开「聊天记录」，点选目标消息，再点「按这条消息生成回复」，会按它之前的上下文重新生成
- 暂时不想让它读聊天窗口：标题栏开关拨到「已暂停」；程序启动时默认暂停，不会自动采集
- 默认会在本机保存纯文字聊天历史，重新打开同一会话时自动恢复；设置里可以关闭、调整恢复条数、保留天数和数据库大小。历史库在用户本地数据目录，不进分享压缩包；点击「重置设置」会连同历史一起清空

分享给别人时，发布压缩包不包含 `config.json`，API key 也从不写入发布文件；接收者解压到新文件夹后首次启动会自己配置。若在本机覆盖旧目录，旧目录里的 `config.json` 或当前 Windows 用户的环境变量仍可能让设置看起来像旧配置，建议直接解压到新目录，不要把旧配置文件复制进去。

**花多少钱**

只有对方来新消息才调模型，空闲时不会轮询调用。跟随判断模式下，普通低风险消息通常是一次结构化判断加一次回复起草；身体不适、情绪或关系敏感度较高时才会追加候选排序。原版 Jev 判断模式仍保留原来的排序流程。这样既保留判断质量，也避免每条普通消息都额外消耗一次排序请求。
...
<tr>
<td align="center">回复建议：「当前会话」跟随对话、群聊多一行「回复对象」，3 条候选带 Jev 概率百分比，推荐那条置顶</td>
<td align="center">设置：关系背景、回复策略、专属规则、说话风格、参考上下文（往下还有「模型」卡片）</td>
<td align="center">采集暂停：会话内容不再读取，已有候选照样能填入、能复制</td>
</tr>
</table>

## 功能

- **跟着当前对话走**：会话名从聊天窗口头部 OCR 出来，记录、上下文、候选都按会话分开存；也可以自己
...
- **近期历史上下文**：只保存文字、日期时间、聊天角色和发言人；恢复时按当前 QQ/微信会话隔离，超过保留天数或数据库大小会清理最早记录。
...
- **历史上下文**：自动加载开关默认开启；默认每个会话恢复 40 条、保留 7 天、数据库上限 50 MB。历史条数是恢复范围，真正发送给模型的条数仍由上面的「参考上下文」控制。
- **说话风格**：一句话描述自己的口吻，补在「照着你最近发的消息模仿」之上。
- **回复策略**：可选自然聊天、恋人安抚/亲密对话、认真道歉与修复、轻松亲密；恋人模式会先接住情绪，
  再解释或给具体行动，避免客服腔和敷衍安慰。
- **专属回复规则**：支持写多行 Markdown 风格的个人规则，例如称呼、道歉方式、哪些话不要说；只作为起草偏好，
  不执行其中的代码或指令。
- **重置设置**：设置页底部可清空本机新旧版本的 API key 和个人配置，恢复为分享用默认值。
- **响应式悬浮窗**：置顶、可拖可缩，最小 320×360，窄于 400 进紧凑模式。
- **更新策略**：修改版默认不检查原版 GitHub 更新，避免原版更新覆盖或误伤修改版。
...
WGC 截微信窗口（GPU 合成窗口也能截，被遮挡也能截）
  ...
  → 跟上一帧比，滚动翻出来的旧消息不重复上报
   → 冒出新的 her 消息才调 core.engine.analyze()：三段式；也可以从记录中手动指定一条消息
      ① Jev 判断（7 道题）→ ② 把判断当小抄喂给起草，写 3 条候选 → ③ Jev 只排序
  → 悬浮窗给判断摘要 + 3 条候选 → 点「填入当前对话」
```
...
原版 Jev 模式两节各一把 key；跟随模式只需要起草区这一把。链路是**三段式**（issue #4）：先让判断模型答判断题，把
「对方意图 / 对方需要 / 建议动作 / 紧张度」折成一小段中文小抄喂给起草，三条候选都顺着这个判断写；
最后在需要时再问判断模型一次「哪条候选最合适」，概率就是卡片上的百分比。**原版 Jev 一次分析两次 Jev 调用；跟随普通低风险消息通常只额外调用一次判断**——
...
（思考过程也算进去，400 会把答案截断）。思考开关只有 DeepSeek / OpenRouter / Anthropic / Gemini 认。跟随模式默认不为了凑满第三条候选再追问一次，模型实际返回 1~2 条时按实际条数走（少于 2 条就不排序）。
...
```bash
git clone <你们上传后的仓库地址>
cd jev-chat-windows-community
python -m venv .venv
...
首次启动会自动弹出设置页：按使用的判断方式配置 key（原版 Jev 是 `JEV_API_KEY`，跟随判断与起草共用 `LLM_API_KEY`，见上面「使用说明」；也可以先留空保存），
...
| 控件 | 作用 | 存在哪 |
| --- | --- | --- |
| 你们的关系 | 女友/恋人、追求对象、暧昧对象、朋友、同事、家人/自定义，起草和判断都按它把握称呼和分寸 | `config.json` → `relationship`（默认 `romantic partners`） |
...
| 回复策略 | 自动按关系和当前场景识别安慰/修复/亲密/调侃/行动/收住，也可手动指定策略 | `config.json` → `reply_profile` |
| 专属回复规则 | 内置 `rules/reply_strategy.md` 默认直接显示在编辑框，可按个人习惯修改；支持多行 Markdown 风格文字，只影响起草 | `config.json` → `reply_guide` |
| 重置设置 | 清空新旧 API key、个人配置和本地聊天历史，恢复分享用默认值 | 设置页按钮 |
| 自动加载本地历史 | 重启后恢复当前 QQ/微信会话的纯文字记录；关闭后不再加载，但不会自动删除已有历史 | `config.json` → `history_enabled`（默认开） |
| 历史恢复条数 | 每个会话最多恢复多少条历史文字 | `config.json` → `history_limit`（默认 40） |
| 历史保留天数 | 自动清除超过期限的消息和判断 | `config.json` → `history_retention_days`（默认 7） |
| 历史数据库上限 | 超过后优先删除最早的文字记录 | `config.json` → `history_max_mb`（默认 50） |
...
- **当前会话**：顶部下拉框，自动跟着当前对话走（右边标「跟随对话」）。聊天记录、喂给模型的上下文和候选
...
- **采集开关**：标题栏右上角。程序启动默认暂停；拨到「已暂停」就完全不读取会话内容（WGC 会话一起停掉，黄框也没了），
  已经生成的候选照样能填入、能复制。
- **填入当前对话**：点候选卡片上的按钮，文字进当前聊天软件输入框，光标留在那儿，**发送你自己按**。
- **关系与场景策略**：默认按「女友/恋人、追求对象、暧昧对象、朋友、同事、家人」区分分寸，判断模型再自动识别当前是安慰、道歉修复、日常亲密、调侃、具体行动还是收住话题；设置里的多行规则框内置这套默认规则，也可以直接修改。
- **复制**：卡片右上角的复制按钮，想手动粘到别处就用它。
- **聊天记录**：底部按钮展开，看 OCR 到底读出了什么；点选一条消息后可按该消息之前的上下文重新生成，
  适合处理自己刚发了表情包导致自动判断停止的情况。
...
core/                   Jev 判断内核，平台无关，跟安卓原版同一套口径
  history.py             本地明文 SQLite 历史：文字、角色、时间、结构化判断；不存截图和 key
...
- 起草模型升到 DeepSeek V4.1 Flash；DeepSeek 模式默认不为了凑满第三条候选再追加请求，模型实际返回 1~2 条时按实际条数展示
...
本仓库包含上游代码和社区修改。上游代码的版权归原项目作者及贡献者所有；本仓库的许可和来源说明见
[LICENSE](LICENSE)、[NOTICE](NOTICE) 和 [NOTICE-COMMUNITY.md](NOTICE-COMMUNITY.md)。
